          ...
          t.callExpression(
            t.memberExpression(
              t.callExpression(t.identifier('require'), [
                t.stringLiteral('@kaminrunde/firescout-utils'),
              ]),
...
function extractAndRemoveComment(path, query) {
  ...
  const comment = path.node.leadingComments.find((node) => node.value.includes(query))
  if (!comment) return null
  path.node.leadingComments = path.node.leadingComments.filter((node) => node !== comment)
  return comment
}
