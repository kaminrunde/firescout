  ...
  return {
    ...
    nth: function (n) {
      if (!elements[n]) {
        utils.bubbleError(
          2,
          '"nth(' + n + ')" does not work on a list of length ' + elements.length
        )
      }
      return wrap([elements[n]], ctx)
    },
    wait: function (ms) {
      return ctx.act(function () {
