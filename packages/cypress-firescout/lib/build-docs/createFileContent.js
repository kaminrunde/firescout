...
function createFileContent(tree) {
    return ("\n    /// <reference types=\"cypress\" />\n\n    " + tree.map(function (node) { return "\n      interface " + node.typesaveContext + " extends Cypress.Chainable<Element> {\n        " + node.triggers.map(function (trigger) {
        var _a, _b;
        return "\n          /** \n           * " + (((_b = (_a = node.docs) === null || _a === void 0 ? void 0 : _a.triggers.bullets.find(function (row) { return row.name === trigger.name; })) === null || _b === void 0 ? void 0 : _b.value) || '') + "\n           * @name " + trigger.name + "\n           * @file " + trigger.file + "\n           */\n          trigger(name:'" + trigger.name + "', index?:number|string): Cypress.Chainable<Element>\n        ";
