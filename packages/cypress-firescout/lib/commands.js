...
Cypress.Commands.add("widget", { prevSubject: 'optional' }, function (subject, name, index) {
    var cmd;
    if (subject)
        cmd = cy.get(subject.selector + " [data-cy-id=\"" + name + "\"]");
    else
        cmd = cy.get("[data-cy-id=\"" + name + "\"]");
...
Cypress.Commands.add("hasFlag", { prevSubject: 'optional' }, function (subject, name) {
    return cy.get(subject).should('contain.html', "data-cy-flag=\"" + name + "\"");
});
Cypress.Commands.add("notHasFlag", { prevSubject: 'optional' }, function (subject, name) {
