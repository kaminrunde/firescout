...
function structureToDocs(structure) {
  ...
  var states = structure.children.find(function (c) {
    return c.name.toLowerCase() === 'states'
  })
  var handles = structure.children.find(function (c) {
    return c.name.toLowerCase() === 'handles'
  })
  var collections = structure.children.find(function (c) {
    return c.name.toLowerCase() === 'collections'
  })
  if (states) {
    var ast_1 = markdown_ast_1.default(states.content)
    var descNodes = ast_1.filter(function (node) {
  ...
  if (handles) {
    ...
    var descNodes = ast_2.filter(function (node) {
      return node.type !== 'list'
    })
...
function getBullets(nodes) {
  var bulletNodes = nodes.filter(function (node) {
    return node.type === 'list'
  })
  ...
  function parseBullet(node) {
    ...
    var bullets =
      ((_a = subMap.get(node)) === null || _a === void 0 ? void 0 : _a.map(parseBullet)) || null
    if (value.startsWith(': ')) value = value.replace(': ', '')
    return { name: name, value: value, _name: _name, _value: _value, bullets: bullets }
  }
  return bulletNodesFiltered.map(function (node) {
    return parseBullet(node)
  })
}
function createStructure(item, collections) {
  ...
  sections
    .filter(function (s) {
      return s.name.toLowerCase() === 'collections'
    })
    .map(function (section) {
      var ast = markdown_ast_1.default(section.content)
      var bulletNodes = ast.filter(function (node) {
        return node.type === 'list'
      })
      bulletNodes.map(function (node) {
        ...
        var childItem = collections.find(function (item) {
