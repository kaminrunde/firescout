...
exports.codes = {
  ...
  HANDLE_WITHOUT_PARENT: function (item) {
    return [item.payload, item.file]
  },
  STATE_WITHOUT_PARENT: function (item) {
    return [item.payload, item.file]
  },
  COLLECTION_WITHOUT_PARENT: function (item) {
    return [item.payload, item.file]
  },
  HANDLE_HAS_NO_DOCS: function (item) {
    return [item.name, item.file]
  },
  STATE_HAS_NO_DOCS: function (item) {
    return [item.name, item.file]
  },
  HANDLE_HAS_NO_REF: function (item) {
    return [item.name, item.file]
  },
  STATE_HAS_NO_REF: function (item) {
    return [item.name, item.file]
  },
  NO_DOCS: function (item) {
    return [item.name, item.file]
  },
  COLLECTION_HAS_NO_REF: function (item) {
    return [item.name, item.file]
  },
  COLLECTION_HAS_NO_DOCS: function (item) {
    return [item.name, item.file]
  },
  NO_CTX_REF: function (item) {
    return [item.name, item.file]
  },
  STATE_IMPLEMENTATION_HAS_NO_REF: function (item) {
    return [item.name, item.file]
  },
  STATE_IMPLEMENTATION_HAS_NO_DOCS: function (item) {
    return [item.name, item.file]
  },
  MIXED_STATES_AND_IMPLEMENTATIONS: function (item) {
    return [item.name, item.file]
  },
}
function report(code, ctx) {
  var _a = exports.codes[code](ctx),
    name = _a[0],
    file = _a[1]
  console.log(
    safe_1.default.yellow(code),
    safe_1.default.green(name),
    safe_1.default.grey(file.slice(1))
  )
}
...
function validate(input) {
  ...
  for (var _i = 0, _a = Array.from(treeCtx); _i < _a.length; _i++) {
    var _b = _a[_i],
      name_1 = _b[0],
      file = _b[1]
    if (!docsCtx.has(name_1)) report('NO_DOCS', { name: name_1, file: file })
  }
  for (var _c = 0, _d = Array.from(treeCollections); _c < _d.length; _c++) {
    var _e = _d[_c],
      name_2 = _e[0],
      file = _e[1]
    if (!docsCollections.has(name_2)) report('COLLECTION_HAS_NO_DOCS', { name: name_2, file: file })
  }
  for (var _f = 0, _g = Array.from(treeHandles); _f < _g.length; _f++) {
    var _h = _g[_f],
      name_3 = _h[0],
      file = _h[1]
    if (!docsHandles.has(name_3)) report('HANDLE_HAS_NO_DOCS', { name: name_3, file: file })
  }
  for (var _j = 0, _k = Array.from(treeStates); _j < _k.length; _j++) {
    var _l = _k[_j],
      name_4 = _l[0],
      file = _l[1]
    if (!docsStates.has(name_4)) report('STATE_HAS_NO_DOCS', { name: name_4, file: file })
  }
  ...
  for (var _q = 0, _r = Array.from(docsCtx); _q < _r.length; _q++) {
    var _s = _r[_q],
      name_6 = _s[0],
      file = _s[1]
    if (!treeCtx.has(name_6)) report('NO_CTX_REF', { name: name_6, file: file })
  }
  for (var _t = 0, _u = Array.from(docsCollections); _t < _u.length; _t++) {
    var _v = _u[_t],
      name_7 = _v[0],
      file = _v[1]
    if (!treeCollections.has(name_7)) report('COLLECTION_HAS_NO_REF', { name: name_7, file: file })
  }
  for (var _w = 0, _x = Array.from(docsHandles); _w < _x.length; _w++) {
    var _y = _x[_w],
      name_8 = _y[0],
      file = _y[1]
    if (!treeHandles.has(name_8)) report('HANDLE_HAS_NO_REF', { name: name_8, file: file })
  }
  for (var _z = 0, _0 = Array.from(docsStates); _z < _0.length; _z++) {
    var _1 = _0[_z],
      name_9 = _1[0],
      file = _1[1]
    if (!treeStates.has(name_9)) report('STATE_HAS_NO_REF', { name: name_9, file: file })
  }
