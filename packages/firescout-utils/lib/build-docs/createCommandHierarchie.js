...
function createCommandHierarchie(rawItems) {
  ...
  for (var _a = 0, collectionItems_1 = collectionItems; _a < collectionItems_1.length; _a++) {
    ...
    var _b = extractItems(collectionItem, handleItems),
      includeHandles = _b[0],
      excludeHandles = _b[1]
    var _c = extractItems(collectionItem, stateItems),
      includeStates = _c[0],
      excludeStates = _c[1]
    var _d = extractItems(collectionItem, enhancedCollections),
      includeColls = _d[0],
      excludeColls = _d[1]
  ...
  for (var _e = 0, ctxItems_1 = ctxItems; _e < ctxItems_1.length; _e++) {
    ...
    var _f = extractItems(ctx, handleItems),
      includeHandles = _f[0],
      excludeHandles = _f[1]
    var _g = extractItems(ctx, stateItems),
      includeStates = _g[0],
      excludeStates = _g[1]
    var _h = extractItems(ctx, enhancedCollections),
      includeColls = _h[0],
      excludeColls = _h[1]
    handleItems = excludeHandles
    stateItems = excludeStates
  ...
  if (enhancedCollections.length)
    for (
      var _l = 0, enhancedCollections_1 = enhancedCollections;
      _l < enhancedCollections_1.length;
      _l++
    ) {
