...
export default function createCommandHierarchie(rawItems: RawItem[]): {
  ...
  for (let collectionItem of collectionItems) {
    const [includeHandles, excludeHandles] = extractItems(
      collectionItem,
      handleItems
    );
    const [includeStates, excludeStates] = extractItems(
      collectionItem,
      stateItems
    );
    const [includeColls, excludeColls] = extractItems(
      collectionItem,
      enhancedCollections
    );
  ...
  if (handleItems.length)
    for (let item of handleItems) report("HANDLE_WITHOUT_PARENT", item);
  if (stateItems.length)
    for (let item of stateItems) report("STATE_WITHOUT_PARENT", item);
  if (enhancedCollections.length)
    for (let item of enhancedCollections)
      report("COLLECTION_WITHOUT_PARENT", item);
...
function extractItems(target: RawItem, rawItems: any[]): [any[], any[]] {
  ...
  for (let item of rawItems) {
    if (
      item.folder === target.folder ||
      item.folder.includes(target.folder + "/")
    )
      include.push(item);
